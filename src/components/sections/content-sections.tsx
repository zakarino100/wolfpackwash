        ...
        <div className="mt-8 grid gap-4 lg:grid-cols-2">
          {items.map((item, index) => (
            <Reveal key={item.title} delay={index * 0.08}>
              <Link href={item.href} className="block rounded-[2rem] border border-white/10 bg-white/5 p-6 transition hover:border-amber-300/40 hover:bg-white/[0.07]">
...
export function CtaBand() {
  return (
    <section className="bg-black py-16 text-white sm:py-20">
      <div className="mx-auto max-w-7xl rounded-[2rem] border border-amber-300/20 bg-[linear-gradient(135deg,rgba(200,25,32,0.18),rgba(255,255,255,0.03))] px-6 py-10 text-center sm:px-8">
        <Reveal>
          ...
          <div className="mt-8 flex flex-col items-center justify-center gap-3 sm:flex-row">
            <Link href={siteConfig.primaryCta.href} className="inline-flex rounded-full bg-amber-400 px-6 py-3 text-sm font-semibold text-black transition hover:bg-amber-300">
